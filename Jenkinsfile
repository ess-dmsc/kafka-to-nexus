                ...
                stage('${image_key} Test') {
                    ...
                    if (image_key == test_and_coverage_os) {
                        def coverage_script = """
                            ...
                            lcov --remove coverage.info '*_generated.h' 'src/date/*' '*/.conan/data/*' --output-file coverage_filtered.info
                        """
                        ...
                        sh "docker cp ${container_name(image_key)}:/home/jenkins/build/coverage.info ./coverage_filtered.info"
                        junit "${test_output}"

                        sh "curl -O https://raw.githubusercontent.com/eriwen/lcov-to-cobertura-xml/master/lcov_cobertura/lcov_cobertura.py"
                        sh "python lcov_cobertura.py coverage_filtered.info"
                        step([
