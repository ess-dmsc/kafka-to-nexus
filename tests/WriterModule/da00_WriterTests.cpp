...
static ::testing::AssertionResult NodeHasData(
  ...
  if constexpr (std::is_integral_v<T>) {
    ...
    for (size_t i = 0; i < data.size(); ++i) {
      passed &=
          std::abs(static_cast<double>(data[i] - read_data[i])) <= 1e-6 * std::abs(static_cast<double>(read_data[i] + data[i]));
    }
  }
  if (!passed) {
...
public:
  ...
  // These fields are now marked private, so cannot be accessed directly
  // But they're also not used in any test context, so we can just ignore them
