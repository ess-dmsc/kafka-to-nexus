...
FileWriter::FlatbufferMessage create_f144_message(std::string const &source, double value, int64_t timestamp_ms) {
  auto const [buffer, size] = create_f144_message_double(source, value, timestamp_ms);
...
TEST(SourceFilter, messages_with_same_timestamp_allowed_when_allowed_repeated_is_true) {
...
TEST(SourceFilter, last_message_before_start_time_is_allowed_through_after_valid_message) {
  // For values that don't update very often, the forwarder periodically sends the current value to Kafka.
  // This is to ensure that the data file contains the value despite it not changing.
  auto writer = std::make_unique<StubMessageWriter>();
  auto registrar = std::make_unique<Metrics::Registrar>("");
  auto f144_writer = std::make_unique<WriterModule::f144::f144_Writer>();
  Stream::SourceFilter filter{time_point{1000ms}, time_point::max(), false, writer.get(), std::move(registrar)};
...
TEST(SourceFilter, last_message_before_start_time_handles_out_of_order_messages) {
  // For values that don't update very often, the forwarder periodically sends the current value to Kafka.
  // This is to ensure that the data file contains the value despite it not changing.
  auto writer = std::make_unique<StubMessageWriter>();
  auto registrar = std::make_unique<Metrics::Registrar>("");
...
TEST(SourceFilter, last_message_before_start_time_is_allowed_through_on_destruction_if_no_updates) {
  // For values that don't update very often, the forwarder periodically sends the current value to Kafka.
  // This is to ensure that the data file contains the value despite it not changing.
  auto writer = std::make_unique<StubMessageWriter>();
  auto registrar = std::make_unique<Metrics::Registrar>("");
...
TEST(SourceFilter, last_message_before_start_time_is_not_allowed_through_on_destruction_if_already_written) {
  // For values that don't update very often, the forwarder periodically sends the current value to Kafka.
  // This is to ensure that the data file contains the value despite it not changing.
  auto writer = std::make_unique<StubMessageWriter>();
  auto registrar = std::make_unique<Metrics::Registrar>("");
