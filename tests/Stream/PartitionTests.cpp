...
public:
  PartitionStandIn(std::unique_ptr<Kafka::ConsumerInterface> Consumer,
                   ...
                   [[maybe_unused]] Stream::SrcToDst const &Map,
                   [[maybe_unused]] Stream::MessageWriter *Writer,
                   [[maybe_unused]] time_point Start, time_point Stop,
                   duration StopLeeway, duration KafkaErrorTimeout,
                   std::function<bool()> AreStreamersPausedFunction)
      : Stream::Partition(
            std::move(Consumer), Partition, std::move(TopicName), {},
            std::make_unique<Metrics::Registrar>("some_prefix").get(), Stop,
            StopLeeway, KafkaErrorTimeout, AreStreamersPausedFunction) {}
