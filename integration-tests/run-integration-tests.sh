...
docker exec ${FILEWRITER_FILEWRITER_CONTAINER_NAME:-filewriter} bash -c "cd integration-tests; scl enable rh-python38 -- ~/venv/bin/pytest --file-writer-binary=../kafka-to-nexus --kafka-broker=${FILEWRITER_KAFKA_CONTAINER_NAME:-kafka}:9093 --junitxml=IntegrationTestsOutput.xml"
