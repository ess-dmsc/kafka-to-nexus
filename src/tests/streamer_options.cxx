...
public:
  ...
  void CompareRdKafkaOptionsWith(const std::vector<std::pair<std::string,std::string>>&);
  void CompareSteamerOptionsWith(const FileWriter::StreamerOptions&);
  void CompareSteamerOptionsWith(const milliseconds&, const milliseconds&,
                                 const int&);
...
protected:
  ...
  void StreamerOptionsTest::CompareRdKafkaOptionsWith(const FileWriter::StreamerOptions& Other) {
    EXPECT_EQ(Options.RdKafkaOptions,Other.RdKafkaOptions);
    
  }
  void StreamerOptionsTest::CompareRdKafkaOptionsWith(const std::vector<std::pair<std::string,std::string>>& v) {
    EXPECT_EQ(Options.RdKafkaOptions,v);
  }
  
  void StreamerOptionsTest::CompareSteamerOptionsWith(const FileWriter::StreamerOptions& Other) {
  ...
  void StreamerOptionsTest::CompareSteamerOptionsWith(const milliseconds& ConsumerTimeout,
                                                      const milliseconds& BeforeStartTime,
...
TEST_F(StreamerOptionsTest,set_streamer_options_matches_expected) {
  ...
  ConfigureStreamerWithJson(optj);
  CompareSteamerOptionsWith(milliseconds(10),milliseconds(10),1);
} 
