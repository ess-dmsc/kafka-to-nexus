...
using offset_list = std::vector<std::pair<int,int64_t>>;
  MAKE_CONST_MOCK5(getOffsetForTimeInternal, offset_list(std::string, std::string,
                                                                            std::vector<int>, Stream::time_point,
                                                                            Stream::duration), override);
  MAKE_CONST_MOCK3(getPartitionsForTopicInternal, std::vector<int>(std::string, std::string,
                                                   Stream::duration), override);
  MAKE_MOCK2(getPartitionsForTopic, void(KafkaW::BrokerSettings, std::string), override);
  MAKE_MOCK3(getOffsetsForPartitions, void(KafkaW::BrokerSettings, std::string, std::vector<int>), override);
  MAKE_MOCK3(createStreams, void(KafkaW::BrokerSettings, std::string,
    std::vector<std::pair<int, int64_t>>), override);
  void initMetadataCalls(KafkaW::BrokerSettings, std::string) override {}
  void initMetadataCallsBase(KafkaW::BrokerSettings Settings, std::string Topic) {
    Topic::initMetadataCalls(Settings, Topic);
  }

  void getPartitionsForTopicBase(KafkaW::BrokerSettings Settings, std::string Topic) {
    Topic::getPartitionsForTopic(Settings, Topic);
  }

  void getOffsetsForPartitionsBase(KafkaW::BrokerSettings Settings, std::string Topic, std::vector<int> Partitions) {
    Topic::getOffsetsForPartitions(Settings, Topic, Partitions);
  }

  void createStreamsBase(KafkaW::BrokerSettings Settings, std::string Topic,
...
public:
  auto createTestedInstance() {
    return std::make_unique<TopicStandIn>(KafkaSettings, "some_topic", Map, nullptr, Registrar, Start, 5s, Stop, 5s);
  }
...
TEST_F(TopicTest, StartMetaDataCall) {
  ...
  UnderTest->Executor.SendLowPrioWork([&Promise](){
...
TEST_F(TopicTest, GetPartitionsForTopicException) {
  ...
  REQUIRE_CALL(*UnderTest, getPartitionsForTopic(_, TopicName)).TIMES(1);
  REQUIRE_CALL(*UnderTest, getPartitionsForTopicInternal(_, TopicName, _)).TIMES(1).THROW(MetadataException("Test"));
...
TEST_F(TopicTest, GetPartitionsForTopicNoException) {
  ...
  REQUIRE_CALL(*UnderTest, getPartitionsForTopicInternal(_, TopicName, _)).TIMES(1).RETURN(ReturnPartitions);
  REQUIRE_CALL(*UnderTest, getOffsetsForPartitions(_, TopicName, ReturnPartitions)).TIMES(1);
  UnderTest->getPartitionsForTopicBase(KafkaSettings, TopicName);
...
TEST_F(TopicTest, GetOffsetsForPartitionsException) {
  ...
  REQUIRE_CALL(*UnderTest, getOffsetsForPartitions(_, TopicName, Partitions)).TIMES(1);
  REQUIRE_CALL(*UnderTest, getOffsetForTimeInternal(_, TopicName, Partitions, _, _)).TIMES(1).THROW(MetadataException("Test"));
...
TEST_F(TopicTest, GetOffsetsForPartitionsNoException) {
  ...
  REQUIRE_CALL(*UnderTest, getOffsetForTimeInternal(_, TopicName, Partitions, _, _)).TIMES(1).RETURN(ReturnOffsets);
...
TEST_F(TopicTest, CreateStreams) {
  ...
  for (size_t i = 0; i < PartitionOffsets.size(); i++) {
    EXPECT_EQ(UnderTest->ConsumerThreads[i]->getPartitionID(), PartitionOffsets[i].first);
