...
namespace FileWriter {
  ...
  public:
    void SetUp() override {
      std::map<std::string, ModuleFactory> &WriterFactories = HDFWriterModuleRegistry::getFactories();
  ...
  TEST_F(WriterRegistrationTest, SimpleRegistration) {
    std::map<std::string, ModuleFactory> &Factories = HDFWriterModuleRegistry::getFactories();
  ...
  TEST_F(WriterRegistrationTest, SameKeyRegistration) {
    ...
    EXPECT_THROW(HDFWriterModuleRegistry::Registrar<DummyWriter> RegisterIt(TestKey), std::runtime_error);
  }
