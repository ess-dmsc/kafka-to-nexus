...
public:
  void SetUp() override {
    ...
    WriterModule::Registry::Registrar<WriterModule::f142::f142_Writer> RegisterIt1("f142",
                                                                       "f142");
    WriterModule::Registry::Registrar<WriterModule::hs00::hs00_Writer> RegisterIt2("hs00",
                                                                                  "hs00");
  }
};
...
TEST_F(JobCreator, ExtractStreamSettingsEmptyModule) {
  ...
  EXPECT_THROW(FileWriter::extractStreamInformationFromJsonForSource(TestConfig), std::runtime_error);
}
...
TEST_F(JobCreator, ExtractStreamSettings) {
  ...
  "topic": "my_test_topic"})""};
  StreamHDFInfo TestConfig{ModuleId, HDF5Parent, Config};
  auto Result = FileWriter::extractStreamInformationFromJsonForSource(TestConfig);
  EXPECT_EQ(Result.Module, ModuleId);
  EXPECT_EQ(Result.Topic, "my_test_topic");
  EXPECT_EQ(Result.Source, "my_test_pv");
  EXPECT_EQ(nlohmann::json::parse(Result.ConfigStreamJson), nlohmann::json::parse(Config));
}
...
TEST_F(JobCreator, Generatef142Writer) {
  ...
  EXPECT_TRUE(dynamic_cast<WriterModule::f142::f142_Writer*>(WriterInstance.get()) != nullptr);
}
...
TEST_F(JobCreator, SetWriterAttributes) {
  auto TestFile = HDFFileTestHelper::createInMemoryTestFile("testFile.hdf5", false);
  std::string ModuleId{"f142"};
  std::string HDF5Parent{"/entry/"};
  std::string Config{R""({
              "dtype": "double",
  ...
  "topic": "my_test_topic"})""};
  StreamHDFInfo TestConfig{ModuleId, HDF5Parent, Config};
  auto StreamInfo = FileWriter::extractStreamInformationFromJsonForSource(TestConfig);
