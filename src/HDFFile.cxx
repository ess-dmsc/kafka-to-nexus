...
#include "amor_event-schema_generated.h"
#include "logger.h"
// remove later
#include <iostream>
...
HDFFile::HDFFile() { impl.reset(new HDFFile_impl); }
...
class sinq_event : public FBSchemaReader {
public:
  sinq_event();
  std::unique_ptr<FBSchemaWriter> create_writer_impl();
  std::string sourcename_impl(char *msg_data);
  uint64_t ts_impl(char *msg_data);
  uint64_t pid_impl(char *msg_data);
};
}
...
class sinq_event : public FBSchemaWriter {
public:
  sinq_event();
  void init_impl(std::string const &sourcename, char *msg);
  WriteResult write_impl(char *msg_data);
...
std::string f140_general::sourcename_impl(char *msg_data) {
  auto pv = BrightnESS::ForwardEpicsToKafka::FlatBufs::f140_general::GetPV(
      msg_data + 2);
  return pv->src()->str();
}
...
std::string f141_ntarraydouble::sourcename_impl(char *msg_data) {
  auto pv =
      BrightnESS::ForwardEpicsToKafka::FlatBufs::f141_ntarraydouble::GetPV(
          msg_data + 2);
  return pv->src()->str();
}
...
sinq_event::sinq_event() {}
std::unique_ptr<FBSchemaWriter> sinq_event::create_writer_impl() {
  return std::unique_ptr<FBSchemaWriter>(new schemawriters::sinq_event);
}
std::string sinq_event::sourcename_impl(char *msg_data) {
  auto event = BrightnESS::EventGenerator::FlatBufs::AMOR::GetEvent(msg_data);
  return event->htype()->str();
}
uint64_t sinq_event::ts_impl(char *msg_data) {
  auto event = BrightnESS::EventGenerator::FlatBufs::AMOR::GetEvent(msg_data);
  return event->ts();
}
uint64_t sinq_event::pid_impl(char *msg_data) {
  auto event = BrightnESS::EventGenerator::FlatBufs::AMOR::GetEvent(msg_data);
  return event->pid();
}
}
...
WriteResult f140_general::write_impl(char *msg_data) { return {-1}; }
...
void f141_ntarraydouble::init_impl(std::string const &sourcename,
                                   char *msg_data) {
  // TODO
  ...
  if (true) {
    ...
    for (int i1 = 0; i1 < ndims; ++i1) {
      LOG(0, "H5Sget_simple_extent_dims {:3} {:3}", get_sizes_now.at(i1),
          get_sizes_max.at(i1));
    }
  }
  ...
  std::array<hsize_t, 2> sizes_chk{
      std::max(64 * 1024 / H5Tget_size(dt) / ncols, (size_t)1), ncols};
...
WriteResult f141_ntarraydouble::write_impl(char *msg_data) {
  ...
  }
  {
    ...
    A hsl_count{1, get_sizes_now.at(1)};
    err = H5Sselect_hyperslab(tgt, H5S_SELECT_SET, hsl_start.data(), nullptr,
                              hsl_count.data(), nullptr);
    if (err < 0) {
  ...
  sinq_event::sinq_event() {}
  void sinq_event::init_impl(std::string const &sourcename, char *msg_data) {
    auto event =
      BrightnESS::EventGenerator::FlatBufs::AMOR::GetEvent(msg_data);
    std::cout << sourcename << "\n";
    std::cout << "pid = " << event->pid() << "\t"
              << "[" << event->ts() << "]\t"
              << event->data() << "\n";
  }
  WriteResult sinq_event::write_impl(char *msg_data) { return {-1}; }
}

std::unique_ptr<FBSchemaReader> FBSchemaReader::create(char *msg_data,
                                                       int msg_size) {
  ...
  if (sid == 0xf141) {
    return std::unique_ptr<FBSchemaReader>(
        new schemareaders::f141_ntarraydouble);
  }
  return nullptr;
}
...
uint64_t FBSchemaReader::ts(char *msg_data) { return ts_impl(msg_data); }

FBSchemaWriter::FBSchemaWriter() {}

FBSchemaWriter::~FBSchemaWriter() {}

void FBSchemaWriter::init(HDFFile *hdf_file, std::string const &sourcename,
                          char *msg_data) {
