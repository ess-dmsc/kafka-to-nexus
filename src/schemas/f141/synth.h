...
template <typename T>
struct nt_val_type : public nt_val_type_U {
static _V v();
};

template <> struct nt_val_type<int32_t> { static _V v() { return _V::NTScalarArrayInt;} };
template <> struct nt_val_type<double> { static _V v() { return _V::NTScalarArrayDouble;} };
template <> struct nt_val_type<float> { static _V v() { return _V::NTScalarArrayFloat;} };
...
fb next(uint64_t seq) {
	...
	std::is_same<T, int32_t>::value, NTScalarArrayIntBuilder, typename std::conditional<
	std::is_same<T, double>::value, NTScalarArrayDoubleBuilder, std::nullptr_t
	>::type
	>::type;
