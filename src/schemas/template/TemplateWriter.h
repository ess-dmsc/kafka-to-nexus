...
public:
  ...
  /// and do verification in the write stage instead:
...
public:
  ...
  /// \note Constructor is executed in catch-all block (which re-throws)
  ...
  /// requires that only writing data pre-existing datasets is allowed, this
  /// function must implement this funtionality. A list of rules to follow when
  ...
  /// exceptions here unless you encounter an unrecoverable state as any
  /// exceptions will cause the thread to exit.
  /// \note The return value of this not actually checked in the current
  /// implementation.
  /// \param[in] hdf_parent This is actual HDF5 group in where the relevant
  ...
  /// constructed using a set of static member functions defined in the file
