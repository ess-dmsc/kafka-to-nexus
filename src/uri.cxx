...
static_ini::static_ini() {
  ...
  size_t errpos = 0;
  {
    auto s1 = (uchar *)"^\\s*(([a-z]+):)?//(([-._A-Za-z0-9]+)(:([0-9]+))?)(/"
                       "([-./_A-Za-z0-9]*))?\\s*$";
    auto re =
        pcre2_compile_8(s1, PCRE2_ZERO_TERMINATED, 0, &err, &errpos, nullptr);
    if (!re) {
      p_regerr(err);
  ...
  {
    auto s1 = (uchar *)"^\\s*(([-._A-Za-z0-9]+)(:([0-9]+))?)(/([-./"
                       "_A-Za-z0-9]*))?\\s*$";
    auto re =
        pcre2_compile_8(s1, PCRE2_ZERO_TERMINATED, 0, &err, &errpos, nullptr);
    if (!re) {
...
static_ini::~static_ini() {
  if (auto &x = URI::re1)
    pcre2_code_free(x);
  if (auto &x = URI::re_host_no_slashes)
    pcre2_code_free(x);
  if (auto &x = URI::re_no_host)
    pcre2_code_free(x);
  if (auto &x = URI::re_topic)
    pcre2_code_free(x);
}
...
void URI::update_deps() {
  ...
  if (0 <= pcre2_match(re_topic, (uchar *)path.data(), path.size(), 0, 0, mdd,
                       nullptr)) {
...
void URI::init(std::string uri) {
  ...
  if (!match && !require_host_slashes) {
    int x;
    x = pcre2_match(re_host_no_slashes, (uchar *)uri.data(), uri.size(), 0, 0,
                    mdd, nullptr);
    if (x >= 0) {
      match = true;
      MD m(mdd);
  ...
  if (!match) {
    int x;
    x = pcre2_match(re_no_host, (uchar *)uri.data(), uri.size(), 0, 0, mdd,
                    nullptr);
    if (x >= 0) {
      match = true;
      MD m(mdd);
