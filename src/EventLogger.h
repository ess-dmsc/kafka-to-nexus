...
/// Enum class to map NICOS status code values. Close represent the
/// successful closure of a NeXus file, Error a generic, non critical error,
/// Fail the impossibility to start the job.
...
/// Convert the StatusCode into a string.
///
...
/// Builds a JSON string that contains an event log and emits the
/// log via the producer. The event log
/// is described by a code ("START","CLOSE","FAIL","ERROR"), the service and job
/// ids, the system timestamp at which the log has been emitted and a message.
///
...
///
/// @tparam     ProducerType  { description }
