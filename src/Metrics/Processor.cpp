...
bool Processor::registerMetric(std::string Name, Metrics::CounterType * Counter, std::string Description, Severity LogLevel, Metrics::DestList Targets) {
  if (metricIsInList(Name)) {
    return false;
    Logger->warn("Unable to register metric with name \"{}\" as it is already registered.", Name);
  }
  ...
  if (Targets.find(LogTo::LOG_MSG) != Targets.end()) {
    ...
    Logger->info("Registering the metric \"{}\" for printing as a log message.", Name);
  }
  return true;
}
...
bool Processor::deRegisterMetric(std::string Name) {
  if (not metricIsInList(Name)) {
    Logger->warn("Unable to de-register the metric \"{}\" as it is not a known metric.", Name);
    return false;
  }
  std::lock_guard<std::mutex> LocalLock(MetricsMutex);
  ...
  LogMsgMetrics.erase(std::remove_if(LogMsgMetrics.begin(), LogMsgMetrics.end(), CheckMetricName),
                      LogMsgMetrics.end());
  GrafanaMetrics.erase(std::remove_if(GrafanaMetrics.begin(), GrafanaMetrics.end(), CheckMetricName),
                       GrafanaMetrics.end());
  return true;
}
...
void Processor::threadFunction() {
  ...
  auto findNextStopTime = [](auto const& Now, auto const&PreviousStopTime, auto const &UsedInterval){
  ...
  while (RunThread) {
    auto NextWakeTime = std::min(std::min(NextGrafanaTime, NextLogMsgTime), NextExitThreadCheck);
    ...
    NextExitThreadCheck = findNextStopTime(Now, NextExitThreadCheck, ExitThreadCheckInterval);
  }
}
...
void Processor::generateLogMessages() {
  ...
  for (auto &CMetric : LogMsgMetrics) {
    ...
    if (CurrentValue != CMetric.LastValue) {
      ...
      auto TimeDiff = std::chrono::duration_cast<std::chrono::milliseconds>(Now - CMetric.LastTime).count();
      Logger->log(LogSeverityMap[CMetric.ValueSeverity],"In the past {} ms, {} events of type \"{}\" have occured ({}).", TimeDiff, ValueDiff, CMetric.FullName, CMetric.DescriptionString);
    }
