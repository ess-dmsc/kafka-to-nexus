...
void setCLIOptions(CLI::App &App, MainOpt &MainOptions) {
  ...
  addUriOption(App, "--grafana-carbon-address",
               MainOptions.GrafanCarbonAddress,
               "<host:port> Address to the Grafana (Carbon) metrics service.");
  App.add_option("--metrics-id",
               MainOptions.GrafanaCarbonMetricsId,
               "An extra metrics ID string. Only used if present. Will take the form: \"kafka-to-nexus.[id].*\"");
