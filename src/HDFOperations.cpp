...
void createHDFStructures(
  ...
  try {
    ...
    if (CNode.Type.get_key() == "type") {
      if (CNode.Type.get_value() == "group") {
        ...
        try {
          ...
          LOG_ERROR("Failed to create or access group. Name: {}. Message: {}. "
                    "HDF5 Path: {}",
    ...
    LOG_ERROR("Failed to create structure with path \"{}\" ({} levels deep). "
              "Message: {}",
