...
h5d::~h5d() {
  if (id != -1) {
    ...
    if (not cq) {
      ...
      // TODO
      // This must wait for the other writers to finish!
      // Otherwise the shared snow is not accurate.
      snow[0] = cq->snow[0].load();
      cq->push(CollectiveCommand::set_extent(buf, snow.size(), snow.data()));
    }
    err = H5Dclose(id);
    if (err < 0) {
...
template h5d_chunked_1d<uint32_t>::ptr
h5d_chunked_1d<uint32_t>::open(hid_t loc, string name);
template h5d_chunked_1d<uint64_t>::ptr
h5d_chunked_1d<uint64_t>::open(hid_t loc, string name);
