...
Handler::Handler(std::string ServiceIdentifier, Kafka::BrokerSettings const &Settings, uri::URI JobPoolUri, uri::URI CommandTopicUri) {
  auto JobReceiver = std::make_unique<JobListener>(JobPoolUri, Settings);
  auto Commands = std::make_unique<CommandListener>(CommandTopicUri, Settings);
  auto ResponseHandler = std::make_unique<ResponseProducer>(ServiceIdentifier, CommandTopicUri, Settings);
  Handler(ServiceIdentifier, std::move(JobReceiver), std::move(Commands), std::move(ResponseHandler));
}
...
void Handler::handleCommand(FileWriter::Msg CommandMsg, bool IgnoreServiceId) {
  if (Parser::isStartCommand(CommandMsg)) {
    ...
    LOG_DEBUG("Got command/job of unknown type.");
  }
}
...
void Handler::handleStartCommand(FileWriter::Msg CommandMsg, bool IgnoreServiceId) {
  ...
  if (not IgnoreServiceId and StartJob.ServiceID != ServiceId) {
    CommandResponse->publishResponse(ActionResponse::StartJob, ActionResult::Failure, StartJob.JobID, "Unable to start filewriting job: service id is incorrect.");
    LOG_DEBUG("Rejected start command as the service id was wrong. It should be {}, it was {}.", ServiceId, JobId);
    return;
  }
  if (isJobIdValid(StartJob.JobID)) {
    CommandResponse->publishResponse(ActionResponse::StartJob, ActionResult::Failure, StartJob.JobID, "Unable to start filewriting job: job id was invalid.");
    LOG_DEBUG("Rejected start command as the job id was invalid (it was: {}).", StartJob.JobID);
    return;
  }
  try {
    DoStart(StartJob);
    ...
    LOG_ERROR("Failed to start filewriting job. The failure message was: {}", E.what());
    CommandResponse->publishResponse(ActionResponse::StartJob, ActionResult::Failure, StartJob.JobID, "Unable to start filewriting job. Error message was: " + std::string(E.what()));
    PollForJob = true;
    JobId = "";
    return;
  }
  LOG_INFO("Starting write job.");
  CommandResponse->publishResponse(ActionResponse::StartJob, ActionResult::Success, StartJob.JobID, "");
}
...
void Handler::handleStopCommand(FileWriter::Msg CommandMsg) {
  ...
  if (ServiceId != StopJob.ServiceID or JobId != StopJob.JobID) {
    LOG_DEBUG("Rejected stop command as job and/or service id was incorrect (was {}/{}, should be {}/{}).", StopJob.JobID, StopJob.ServiceID, JobId, ServiceId);
    return;
  }
  if (StopJob.StopTime == 0ms) {
    try {
      ...
      auto ErrorStr = fmt::format("Unable to stop filewriting job immediately. Error message was: {}", E.what());
      CommandResponse->publishResponse(ActionResponse::StopNow, ActionResult::Failure, JobId, ErrorStr);
      LOG_ERROR(ErrorStr);
    }
    CommandResponse->publishResponse(ActionResponse::StopNow, ActionResult::Success, JobId, "");
    LOG_INFO("Attempting to stop writing job now.");
  } else {
    try {
      DoSetStopTime(StopJob.StopTime);
    } catch (std::exception const &E) {
      auto ErrorStr = fmt::format("Unable to set stop time for filewriting job. Error message was: {}", E.what());
      CommandResponse->publishResponse(ActionResponse::SetStopTime, ActionResult::Failure, JobId, ErrorStr);
      LOG_ERROR(ErrorStr);
    }
    CommandResponse->publishResponse(ActionResponse::SetStopTime, ActionResult::Success, JobId, "");
    LOG_INFO("File writing job stop time set to: {}", "Test");
  }
}
