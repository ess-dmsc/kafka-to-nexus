...
ModuleFactory const &find(std::string const &FlatbufferID) {
  ...
  if (FoundFactory == Factories.end()) {
    throw std::out_of_range("Unable to find factory with the id: " + FlatbufferID);
  }
...
void addWriterModule(std::string const &FlatbufferID, std::string const &ModuleName, ModuleFactory Value) {
  ...
  if (Factories.find(ModuleHash) != Factories.end()) {
    auto s = fmt::format("Writer module with name \"{}\" that processes \"{}\" flatbuffers already exists.", ModuleName,
                         FlatbufferID);
