...
template <typename Streamer, typename Demux> struct StreamMaster {
  ...
  // StreamMaster(std::string& broker, std::vector<Demux>& _demux, const
  // RdKafkaOffset& offset = RdKafkaOffsetEnd) : demux(_demux), do_write(false),
  // _stop(false) {
  //   for( auto& d: demux) {
  ...
  // StreamMaster(std::string& broker, std::unique_ptr<FileWriterTask>
  // file_writer_task, const RdKafkaOffset& offset = RdKafkaOffsetEnd) :
