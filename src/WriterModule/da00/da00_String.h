...
public:
  ...
  explicit StringConfig(nlohmann::json const & config) {
    if (config.is_string()) {
      ...
      if (config.contains("value")) _value = config["value"].get<std::string>();
      if (config.contains("size")) _size = config["size"].get<ssize_t>();
    } else {
  ...
  [[nodiscard]] bool knows_value() const {
    return has_value() || has_size();
  }

  template<class ParentUniquePointer>
  hdf5::attribute::Attribute insert_attribute(ParentUniquePointer & parent, std::string const & name) const {
    if (!knows_value()) {
      auto pos = std::string(parent->link().path());
      auto msg = fmt::format("StringConfig can not create attribute {}@{} without value or size", pos, name);
      LOG_ERROR(msg);
    ...
    auto attr = parent->attributes.create(name, output, hdf5::dataspace::Scalar());
    if (has_value()){
      try {
        attr.write(value(), output);
      } catch (std::exception & e){
        LOG_ERROR("Failed to write attribute {}@{}: {}", std::string(parent->link().path()), name, e.what());
      }
    }
    return attr;
  }
  ...
  std::pair<bool, bool> update_from(const StringConfig & other, bool force) {
    ...
    if (other.has_value()){
      if (has_value() && value() != other.value() && force) {
        ...
        _value = other.value();
        if (static_cast<ssize_t>(other.value().size()) > std::max(static_cast<ssize_t>(value().size()), size())){
          inconsistent = true;
        }
        changed = true;
      } else if (has_size() && size() >= static_cast<ssize_t>(other.value().size())) {
        _value = other.value();
        changed = true;
      } else {
        LOG_ERROR("Update StringConfig value too large: {} -> {}", *this, other);
      }
    }
