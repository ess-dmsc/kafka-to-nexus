...
public:
  ...
  EdgeConfig & operator=(std::string const & config) {
    ...
    if (json.is_array()) _edges = json;
    else if (json.is_object()) {
      if (json.contains("first")) _first = json["first"];
      if (json.contains("last")) _last = json["last"];
      if (json.contains("size")) _size = json["size"];
    }
    if (_edges.has_value() || (_first.has_value() && _last.has_value() && _size.has_value()))
      return *this;
    LOG_ERROR(
        "Invalid EdgeConfig JSON input {} -> edges = {}, first = {}, last = {}, size = {}",
        config, _edges.has_value(), _first.has_value(), _last.has_value(), _size.has_value());
  ...
  [[nodiscard]] nlohmann::json const & edges_json() const { return _edges.value(); }
  [[nodiscard]] nlohmann::json const & first_json() const { return _first.value(); }
  [[nodiscard]] nlohmann::json const & last_json() const { return _last.value(); }
  ...
  template<class T> [[nodiscard]] std::vector<T> edges() const {
    std::vector<T> out;
    if (_edges.has_value()) out = _edges.value().get<std::vector<T>>();
    if (out.size() < 2 && _first.has_value() && _last.has_value() && _size.has_value()) {
...
template<> struct fmt::formatter<WriterModule::da00::EdgeConfig> {
  ...
  auto format(const WriterModule::da00::EdgeConfig &edge, FormatContext &ctx) {
    const auto values = edge.has_edges() ? edge.edges_json() : nlohmann::json::array();
    ...
    return format_to(ctx.out(), "EdgeConfig(edge={}, first={}, last={}, size={})",
