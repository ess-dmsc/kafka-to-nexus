...
const char *WriterModule::da00::da00_dtype_to_string(da00_dtype const &type) {
  ...
  case da00_dtype::int8:
    return "int8";
  case da00_dtype::uint8:
    return "uint8";
  case da00_dtype::int16:
    return "int16";
  case da00_dtype::uint16:
    return "uint16";
  ...
  case da00_dtype::uint64:
    return "uint64";
  case da00_dtype::float32:
    return "float32";
  case da00_dtype::float64:
    return "float64";
  case da00_dtype::c_string:
    return "c_string";
  default:
    return "none";
  }
...
da00_dtype WriterModule::da00::guess_dtype(nlohmann::json const &data) {
  ...
  if (data.is_array()) {
    if (data.empty())
      return da00_dtype::none;
    if (data[0].is_boolean())
      return da00_dtype::uint8;
    if (data[0].is_number_integer())
      return da00_dtype::int64;
    if (data[0].is_number_unsigned())
      return da00_dtype::uint64;
    if (data[0].is_number_float())
      return da00_dtype::float64;
    if (data[0].is_string())
      return da00_dtype::c_string;
  }
