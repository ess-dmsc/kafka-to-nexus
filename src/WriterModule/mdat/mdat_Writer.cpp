...
void mdat_Writer::writemetadata(std::string const &name,
  ...
  //  std::cout << "mdatmdat" << buffer << "," << datatime <<std::endl
  //  <<std::endl <<std::endl;
  ...
  // //   auto StringArray =
  // HDFOperations::jsonArrayToMultiArray<std::string>("A");
  // // auto Dims = StringArray.getDimensions();

  // //   auto Dataspace =
  // //       hdf5::dataspace::Simple(hdf5::Dimensions(Dims.begin(),
  // Dims.end())); std::string strBuffer = std::string(buffer); auto StringArray
  // = HDFOperations::jsonArrayToMultiArray<std::string>(strBuffer);
  ...
  //    mdatEnd_datetime.appendArray(hdf5::ArrayAdapter<const char[]>(&buffer,
  //    32));
}
