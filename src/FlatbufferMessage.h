...
public:
  ...
  /// \throws FileWriter::BufferTooSmallError If the size argument is < 8, it can not be a flatbuffer as the identifier is stored in bytes 4 to 8.
  /// \throws FileWriter::UnknownFlatbufferID If the flatbuffer ID (bytes 4 to 8) is not found in the database, this exception is thrown.
  /// \throws FileWriter::NotValidFlatbuffer The constructor runs FileWriter::FlatbufferReader::verify() on the flatbuffer and this exception is thrown if it returns false.
  ...
  /// Get the source name of the flatbuffer. Extracted using FileWriter::FlatbufferReader::source_name().
  /// \return The source name if flatbuffer is valid, an empty string if it is not.
  std::string getSourceName() const { return Sourcename; };
  /// Get the timestamp of the flatbuffer. Extracted using FileWriter::FlatbufferReader::timestamp().
  ...
  /// \return Pointer to flatbuffer data if flatbuffer is valid, `nullptr` if it is not.
  char const *const data() const { return DataPtr; };
  
  /// Get size of flatbuffer.
  /// \return Size of flatbuffer in bytes if flatbuffer is valid, 0 if it is not.
