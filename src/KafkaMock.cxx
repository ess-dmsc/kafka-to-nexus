...
bool Conf::set(const std::string &name, const std::string &value,
               std::string &err) {
  err = "no error";
  return CONF_OK;
}

bool ErrorCode::operator==(const ErrorCode &other) {
...
ErrorCode Consumer::start(Topic *, const int &, const int &) {
  return ErrorCode(0);
}
...
Topic *Topic::create(Consumer *consumer, const std::string &name, Conf *tconf,
                     std::string err) {
  err = "no error";
  return &mock_topic;
}
...
std::string err2str(ErrorCode err) {
  return std::string("error number = ") + std::to_string(int(err));
}
